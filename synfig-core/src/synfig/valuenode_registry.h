...
#define REGISTER_VALUENODE(klass, _version, _name, _local_name)				\
	namespace synfig 														\
	{																		\
		struct Register_##klass												\
		{																	\
			Register_##klass() { 											\
				ValueNodeRegistry::register_node_type(						\
					_name,													\
					ValueNodeRegistry::localize_name(_local_name),			\
					_version			,									\
					reinterpret_cast<ValueNodeRegistry::Factory>(			\
						&klass::create), &klass::check_type					\
				);															\
			}																\
		};																	\
		static const Register_##klass register_##klass = {};				\
																			\
		String																\
